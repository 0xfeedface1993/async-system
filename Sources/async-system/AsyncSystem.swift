...
struct CheckedContinuationSlide<T: Publisher> {
    ...
    func resume(in continuation: CheckedContinuation<T.Output, Error>) -> AnyCancellable {
        ...
        logger.debug("bridge CombineX publisher \(type) to Concurrency Swift hold on \(String(describing: cancellable)).")
        return cancellable
    }
}
...
extension Publisher {
    ...
    private var leagacyAyncValue: Output {
        get async throws {
            ...
            logger.debug("bridge CombineX publisher \(self) dismiss \(String(describing: cancellable)).")
            return result
        }
    }
}
